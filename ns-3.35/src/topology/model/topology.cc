...
Topology::DijkstraShortestPaths (Ptr<Node> src, Ptr<Node> dst)
{
  ...
  for (boost::tie (vertexIt, vertexEnd) = boost::vertices (m_graph); vertexIt != vertexEnd;
       ++vertexIt)
    {
...
int
Topology::CalculateCost (std::vector<Vertex> path)
{
  int cost = 0;
...
int
Topology::CalculateCost (std::vector<Ptr<Node>> path)
{
  int cost = 0;
  for (auto i = path.begin (); i != path.end () - 1; i++)
    {
...
int
Topology::GetEdgeWeight (Vertex v1, Vertex v2)
{
  std::pair<Edge, bool> pair = boost::edge (v1, v2, m_graph);
